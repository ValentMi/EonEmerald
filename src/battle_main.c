...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
    ...
    u8 monsCount;
		...
		u16 dynamicLevel = 0; 
		u8 LevelSpread[] = {0, 0};	// This is used to hold the level's of the player's strongest[1] and weakest[0] Pokemon
		u16 PartyLevelAdjust; // This will be used when assigning the level of the opponent's Pokemon
		...
		/* The following is used to account for a player having one or two very weak Pokemon
			...
			*/
			...
			if(LevelSpread[1] - dynamicLevel < 40)
			{
				PartyLevelAdjust *= 3;
				PartyLevelAdjust /= 10;
			}
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
		...
		if ((gBattleMons[battler1].ability == ABILITY_SAND_RUSH && gBattleWeather & B_WEATHER_SANDSTORM)
            || (gBattleMons[battler1].ability == ABILITY_SLUSH_RUSH && gBattleWeather & B_WEATHER_HAIL) || (gBattleMons[battler1].ability == ABILITY_FORECAST && gBattleWeather & B_WEATHER_HAIL))
            speedMultiplierBattler1 = 2;
        else
            speedMultiplierBattler1 = 1;
		if ((gBattleMons[battler2].ability == ABILITY_SAND_RUSH && gBattleWeather & B_WEATHER_SANDSTORM)
            || (gBattleMons[battler2].ability == ABILITY_SLUSH_RUSH && gBattleWeather & B_WEATHER_HAIL) || (gBattleMons[battler2].ability == ABILITY_FORECAST && gBattleWeather & B_WEATHER_HAIL))
            speedMultiplierBattler2 = 2;
        else
            speedMultiplierBattler2 = 1;
    }
	...
	if ((gBattleMons[battler1].ability == ABILITY_RUN_AWAY) && holdEffect == HOLD_EFFECT_NONE)
		speedBattler1 *= 1.5;
	
	if ((gBattleMons[gActiveBattler].species == SPECIES_SCEPTILE || gBattleMons[battler1].species == SPECIES_HITMONLEE) && holdEffect == HOLD_EFFECT_NONE)
		speedBattler2 *= 1.2;
...
//	if ((gBattleMons[battler1].species == SPECIES_SABLEYE
//	|| gBattleMons[battler1].species == SPECIES_ILLUMISE
//	|| gBattleMons[battler1].species == SPECIES_VOLBEAT
//	|| gBattleMons[battler1].species == SPECIES_MURKROW
//	|| gBattleMons[battler1].species == SPECIES_BANETTE) && gBattleMoves[gCurrentMove].effect == EFFECT_WILL_O_WISP)
//        speedBattler1 *= 10;
	...
	if ((gBattleMons[battler2].ability == ABILITY_RUN_AWAY) && holdEffect == HOLD_EFFECT_NONE)
		speedBattler2 *= 1.5;
	
	if ((gBattleMons[gActiveBattler].species == SPECIES_SCEPTILE || gBattleMons[battler2].species == SPECIES_HITMONLEE) && holdEffect == HOLD_EFFECT_NONE)
		speedBattler2 *= 1.2;
...
//	if ((gBattleMons[battler2].species == SPECIES_SABLEYE
//	|| gBattleMons[battler2].species == SPECIES_ILLUMISE
//	|| gBattleMons[battler2].species == SPECIES_VOLBEAT
//	|| gBattleMons[battler2].species == SPECIES_MURKROW
//	|| gBattleMons[battler2].species == SPECIES_BANETTE) && gBattleMoves[gCurrentMove].effect == EFFECT_WILL_O_WISP)
 //       speedBattler2 *= 10;
