...
static bool8 ShouldSwitchIfWonderGuard(void)
{
    ...
    if (gBattleMons[GetBattlerAtPosition(opposingPosition)].ability == ABILITY_NONE)
        return FALSE;
    ...
    for (opposingBattler = GetBattlerAtPosition(opposingPosition), i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if ((moveFlags & MOVE_RESULT_SUPER_EFFECTIVE) || (move == MOVE_STRUGGLE))
            return FALSE;
    }
...
static bool8 FindMonThatAbsorbsOpponentsMove(void)
{
    ...
    if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_FIRE)
        absorbingTypeAbility = ABILITY_FLASH_FIRE || ABILITY_PERMAFROST;
    else if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_WATER)
        absorbingTypeAbility = ABILITY_WATER_ABSORB;
    else if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_ELECTRIC)
        absorbingTypeAbility = ABILITY_VOLT_ABSORB || ABILITY_LIGHTNING_ROD;
    else if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_ROCK)
        absorbingTypeAbility = ABILITY_ROCK_BODY;
	else if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_GROUND)
        absorbingTypeAbility = ABILITY_LEVITATE || TYPE_FLYING;
    else
        return FALSE;
...
void AI_TrySwitchOrUseItem(void) //Switching
{
    struct Pokemon *party;
    u8 battlerIn1, battlerIn2;
    s32 firstId;
...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    s32 bestDmg;
#else
    s32 bestDmg; // Note: should be changed to s32 since it is also used for the actual damage done later
#endif
    u8 bestMonId;
    u8 battlerIn1, battlerIn2;
    s32 firstId;
