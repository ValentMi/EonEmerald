...
u16 GetCurrLocationDefaultMusic(void)
{
    ...
    //if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(LITTLEROOT_TOWN) //DNS
...
void Overworld_PlaySpecialMapMusic(void)
{
    ...
    if (music != MUS_ABNORMAL_WEATHER && music != MUS_NONE)
    {
        if (gSaveBlock1Ptr->savedMusic)
            ...
            music = MUS_SURF;
//        else if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) && gMapHeader.regionMapSectionId == MAPSEC_CERULEAN_CAVE)//SURFMUSIC
//            music = MUS_RG_SURF;
    }
