...
static void Cmd_setmultihitcounterSkillLink(void);
static void Cmd_setmultihitcounterLoadedDice(void);
...
static const u8 *const sMoveEffectBS_Ptrs[] =
{
    ...
    [MOVE_EFFECT_STEAL_ITEM]       = BattleScript_EffectThief,
...
static const u16 sNaturePowerMoves[] =
{
    ...
    [BATTLE_TERRAIN_POND]       = MOVE_WATER_PULSE,
    [BATTLE_TERRAIN_MOUNTAIN]   = MOVE_ROCK_SLIDE,
    [BATTLE_TERRAIN_CAVE]       = MOVE_SHADOW_BALL,
    [BATTLE_TERRAIN_BUILDING]   = MOVE_WEATHER_BALL,
    [BATTLE_TERRAIN_PLAIN]      = MOVE_RAPID_SPIN
};
...
static bool8 AccuracyCalcHelper(u16 move)
{
    ...
    if ((WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_RAIN) && gBattleMoves[move].effect == EFFECT_THUNDER)
     || (gBattleMoves[move].effect == EFFECT_ALWAYS_HIT || gBattleMoves[move].effect == EFFECT_VITAL_THROW || gBattleMoves[move].effect == EFFECT_FLINCH_MINIMIZE_HIT))
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_KEEN_EYE)
            calc = (calc * 130) / 100; // 1.3 compound eyes boost
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_ILLUMINATE)
            calc = (calc * 120) / 100; // 1.3 compound eyes boost
        if ((gBattleMons[gBattlerAttacker].types[0] == TYPE_POISON || gBattleMons[gBattlerAttacker].types[1] == TYPE_POISON) && gBattleMoves[move].effect == EFFECT_TOXIC)
            calc = (calc * 150) / 100; // POISON TOXIC ACCURACY INCREASE
		if (ABILITY_ON_FIELD2(ABILITY_ILLUMINATE))
			calc = (calc * 120) / 100; // 3x Accuracy boost for team
        ...
        if (gBattleMons[gBattlerAttacker].species == SPECIES_MACHAMP || gBattleMons[gBattlerAttacker].species == SPECIES_SPINDA)
            calc = (calc * 300) / 100; // // No Guard Enemy Accuracy for team
	    if (gBattleMons[gBattlerTarget].species == SPECIES_SPINDA)
            calc = (calc * 80) / 100; // No Guard Enemy Accuracy
	    if (gBattleMons[gBattlerTarget].species == SPECIES_MACHAMP)
            calc = (calc * 300) / 100; // No Guard Enemy Accuracy
        if ((gBattleMons[gBattlerAttacker].species == SPECIES_WIGGLYTUFF) && gBattleMoves[move].effect == EFFECT_SLEEP)
            calc = (calc * 200) / 100; // // No Guard Enemy Accuracy for team
        if ((gBattleMons[gBattlerAttacker].species == SPECIES_DELCATTY) && gBattleMoves[move].effect == EFFECT_SLEEP)
            calc = (calc * 150) / 100; // // No Guard Enemy Accuracy for team
        if ((gBattleMons[gBattlerAttacker].species == SPECIES_ALTARIA) && gBattleMoves[move].effect == EFFECT_SLEEP)
            calc = (calc * 150) / 100; // // No Guard Enemy Accuracy for team
        if ((gBattleMons[gBattlerAttacker].species == SPECIES_ARTICUNO) && gBattleMoves[move].effect == EFFECT_FREEZE_HIT)
            calc = (calc * 130) / 100; // // No Guard Enemy Accuracy for team
        if (gBattleMons[gBattlerAttacker].species == SPECIES_FURRET || gBattleMons[gBattlerAttacker].species == SPECIES_DELIBIRD || gBattleMons[gBattlerAttacker].species == SPECIES_ARIADOS)
            calc = (calc * 130) / 100; // // No Guard Enemy Accuracy for team
        if (gBattleMons[gBattlerAttacker].species == SPECIES_HYPNO && gBattleMoves[move].effect == EFFECT_SLEEP)
            calc = (calc * 200) / 100; // // No Guard Enemy Accuracy for team
...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
				...
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_DEOXYS)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_NINJASK)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_ELECTRODE)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_JOLTEON)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_AERODACTYL)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_MEWTWO)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_CROBAT)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SWELLOW)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_DUGTRIO)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_ALAKAZAM)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SCEPTILE)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SHEDINJA)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_PERSIAN)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_DELCATTY)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SNEASEL)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_RAPIDASH)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SCYTHER)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_PIDGEOT)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_MANECTRIC)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_FURRET)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_LINOONE)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_FLYGON)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_RATICATE)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_LUVDISC)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_YANMA)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_MINUN)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_PLUSLE)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_SHARPEDO)
				+ (gBattleMons[gBattlerAttacker].species == SPECIES_FEAROW)
		
		+ (gBattleMoves[gCurrentMove].effect == EFFECT_SOLAR_BEAM)
                + (holdEffect == HOLD_EFFECT_SCOPE_LENS)
                + 3 * (holdEffect == HOLD_EFFECT_LUCKY_PUNCH && gBattleMons[gBattlerAttacker].species == SPECIES_CHANSEY)
...
static void Cmd_adjustnormaldamage(void)
{
    ...
    if ((holdEffect == HOLD_EFFECT_FOCUS_BAND || gBattleMons[gBattlerTarget].ability == ABILITY_STURDY) && gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
    {
     ...
     && gBattleMons[gBattlerTarget].hp <= gBattleMoveDamage)
    {
        ...
        else if ((gSpecialStatuses[gBattlerTarget].focusBanded) && (gBattleMons[gBattlerTarget].hp != gBattleMons[gBattlerTarget].maxHP))
        {
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if ((gBattleMons[gEffectBattler].ability == ABILITY_SHIELD_DUST || holdEffect == HOLD_EFFECT_REPEL) && !(gHitMarker & HITMARKER_STATUS_ABILITY_EFFECT)
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_FLINCH:
                if (gBattleMons[gEffectBattler].ability == ABILITY_INNER_FOCUS || holdEffect == HOLD_EFFECT_REPEL)
                {
            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                    ...
                    else if (gBattleMons[gBattlerTarget].item && ((gBattleMons[gBattlerTarget].ability == ABILITY_STICKY_HOLD) || (gBattleMons[gBattlerTarget].ability == ABILITY_PICKUP)))
                    {
                        BattleScriptPushCursor();
	...
	gBattlescriptCurrInstr = BattleScript_SpeedUp;
            ...
            case MOVE_EFFECT_KNOCK_OFF:
                ...
                if (gBattleMons[gBattlerTarget].ability == ABILITY_PICKUP)
                {
                    if (gBattleMons[gEffectBattler].item == ITEM_NONE)
                    {
                        gBattlescriptCurrInstr++;
                    ...
                    {
                        gLastUsedAbility = ABILITY_PICKUP;
                        gBattlescriptCurrInstr = BattleScript_StickyHoldActivates;
                        RecordAbilityBattle(gEffectBattler, ABILITY_PICKUP);
                    }
                    break;
                }
...
//SILVER WIND BOOSTED EFFECT FOR MOTHS/BUTTERFLIES
    if ((gBattleMons[gBattlerAttacker].species == SPECIES_BUTTERFREE || gBattleMons[gBattlerAttacker].species == SPECIES_VENOMOTH || gBattleMons[gBattlerAttacker].species == SPECIES_BEAUTIFLY || gBattleMons[gBattlerAttacker].species == SPECIES_DUSTOX || gBattleMons[gBattlerAttacker].species == SPECIES_MASQUERAIN))
...
static void Cmd_playstatchangeanimation(void)
{
	...
	&& gBattleMons[gActiveBattler].ability != ABILITY_PERMAFROST
	...
	&& gBattleMons[gActiveBattler].ability != ABILITY_ROCK_BODY
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
                 ...
                 && notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(0))
        {
		  ...
		  || gBattleMons[gActiveBattler].ability == ABILITY_OBLIVIOUS
		  || gBattleMons[gActiveBattler].ability == ABILITY_PERMAFROST	
		  || gBattleMons[gActiveBattler].ability == ABILITY_GALVANIZED
		  || gBattleMons[gActiveBattler].ability == ABILITY_ROCK_BODY
        ...
        else if ((gBattleMons[gActiveBattler].ability == ABILITY_SHIELD_DUST && flags == 0) || holdEffect == HOLD_EFFECT_REPEL)
        {
            return STAT_CHANGE_DIDNT_WORK;
        }
...
static void Cmd_setmultihitcounter(void)
{
	...
	if ((gBattleMons[gBattlerAttacker].item == ITEM_LUCKY_EGG) || ((gBattleMons[gBattlerAttacker].species == SPECIES_CLOYSTER) || (gBattleMons[gBattlerAttacker].species == SPECIES_AIPOM) || (gBattleMons[gBattlerAttacker].species == SPECIES_HERACROSS))) //(gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level) //SKILL LINK EFFECT
        {
                        gMultiHitCounter = Random() % 4;
            if (gMultiHitCounter > 2)
            {
                gMultiHitCounter = (Random() % 3);
                if (gMultiHitCounter < 4)
                    gMultiHitCounter = 4;
                else
                    gMultiHitCounter = 5;
            }
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        else if ((gBattleMons[gBattlerTarget].ability == ABILITY_STICKY_HOLD) || (gBattleMons[gBattlerTarget].ability == ABILITY_PICKUP))
        {
            gBattlescriptCurrInstr = BattleScript_StickyHoldActivates;
            gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
            RecordAbilityBattle(gBattlerTarget, gLastUsedAbility);
