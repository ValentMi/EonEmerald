...
static void PrintTextOnWindow(u8 windowId, const u8 *string, u8 x, u8 y, u8 lineSpacing, u8 colorId)//Controls all text in the summary screen. Innates
{
    AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, 0, lineSpacing, sTextColors[colorId], 0, string);
}

static void PrintNarrowTextOnWindow(u8 windowId, const u8 *string, u8 x, u8 y, u8 lineSpacing, u8 colorId)//Controls all text in the summary screen. Innates
{
...
{
    u8 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);
    PrintNarrowTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ABILITY), gAbilityDescriptionPointers[ability], 0, 17, 0, 0);
}
...
static void BufferMonTrainerMemo(void)
{
    ...
    {
        //u32 width = GetStringWidth(FONT_SMALL_NARROW, gText_OTSlash, 0);
...
static void PrintMonTrainerMemo(void)//INNATES?
{
    PrintNarrowTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_MEMO), gStringVar4, 0, 1, 0, 0);
}
...
static void PrintRibbonCount(void)
{
    ...
    PrintNarrowTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_RIBBON_COUNT), text, x, 1, 0, 0);
}
...
static void PrintMovePowerAndAccuracy(u16 moveIndex)
{
   	struct Pokemon *mon = &sMonSummaryScreen->currentMon;
		...
		if ((moveIndex == MOVE_HIDDEN_POWER) || (moveIndex == MOVE_SECRET_POWER))
		{
			...
			u8 powerForHiddenPower = (40 * 1) / 63 + 60;
...
static void SetMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (summary->moves[i] != MOVE_NONE) {
            if ((summary->moves[i] == MOVE_HIDDEN_POWER) || (summary->moves[i] == MOVE_SECRET_POWER)) {
...
static void SetNewMoveTypeIcon(void)
{
    ...
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            if ((sMonSummaryScreen->newMove == MOVE_HIDDEN_POWER) || (sMonSummaryScreen->newMove == MOVE_SECRET_POWER)) {
                u8 typeBits  = ((GetMonData(mon, MON_DATA_HP_IV) & 1) << 0)
                     | ((GetMonData(mon, MON_DATA_ATK_IV) & 1) << 1)
                     | ((GetMonData(mon, MON_DATA_DEF_IV) & 1) << 2)
                     | ((GetMonData(mon, MON_DATA_SPEED_IV) & 1) << 3)
