...
//		    if (sFieldMoves[j] != MOVE_SECRET_POWER) // If Mon already knows FLASH, prevent it from being added to action list
		    if (sFieldMoves[j] != MOVE_DIG) // If Mon already knows FLASH, prevent it from being added to action list
    ...
    if (sPartyMenuInternal->numActions < 5 && CanMonLearnTMHM(&mons[slotId], ITEM_TM28 - ITEM_TM01)) // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 9 + MENU_FIELD_MOVES);
//    if (sPartyMenuInternal->numActions < 5 && CanMonLearnTMHM(&mons[slotId], ITEM_TM43 - ITEM_TM01)) // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
...
static void CursorCb_Moves(u8 taskId)
{
	...
	sPartyMenuInternal->exitCallback = NULL;  // Or assign a real callback function here
    Task_ClosePartyMenu(taskId);
}
