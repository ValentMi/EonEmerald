...
u8 TrySetCantSelectMoveBattleScript(void)
{
	...
	if (holdEffect == HOLD_EFFECT_COMBAT_VEST && gBattleMoves[move].power == 0)
    {
        gCurrentMove = move;
        gLastUsedItem = gBattleMons[gActiveBattler].item;
        if (gBattleTypeFlags & BATTLE_TYPE_PALACE)
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if ((holdEffect == HOLD_EFFECT_CHOICE_BAND || holdEffect == HOLD_EFFECT_CHOICE_SPECS || holdEffect == HOLD_EFFECT_CHOICE_SCARF) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
	    // Assault Vest
        if ((holdEffect == HOLD_EFFECT_ASSAULT_VEST && check & MOVE_LIMITATION_TAUNT && gBattleMoves[gBattleMons[battlerId].moves[i]].power == 0))
            unusableMoves |= gBitTable[i];
        if ((holdEffect == HOLD_EFFECT_COMBAT_VEST && check & MOVE_LIMITATION_TAUNT && gBattleMoves[gBattleMons[battlerId].moves[i]].power == 0))
            unusableMoves |= gBitTable[i];
    }
...
u8 AtkCanceller_UnableToUseMove(void)
{
    ...
    do
    {
        ...
        case CANCELLER_THAW: // move thawing
            if (gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE)
            {
                if (gBattleMoves[gCurrentMove].effect == EFFECT_THAW_HIT || EFFECT_BURN_HIT)
                {
                    gBattleMons[gBattlerAttacker].status1 &= ~STATUS1_FREEZE;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_MoveUsedUnfroze;
...
u8 CastformDataTypeChange(u8 battler)
{
    ...
    {
        SET_BATTLER_TYPE1(battler, TYPE_NORMAL);
		SET_BATTLER_TYPE2(battler, TYPE_ELECTRIC);
        ...
        case ABILITYEFFECT_ENDTURN: // 1
            if (gBattleMons[battler].hp != 0)
            {
                ...
                case ABILITY_OVERGROW:
                    ...
                    break;
                    if ((gBattleMons[battler].species == SPECIES_MEGANIUM) && (gBattleMons[battler].status1 & STATUS1_ANY) && (Random() % 3) == 0)
                    {
                        if (gBattleMons[battler].status1 & (STATUS1_POISON | STATUS1_TOXIC_POISON))
                            StringCopy(gBattleTextBuff1, gStatusConditionString_PoisonJpn);
                        if (gBattleMons[battler].status1 & STATUS1_SLEEP)
				...
				case ABILITY_PERMAFROST:
                    ...
                    if (gBattleWeather & B_WEATHER_HAIL && gBattleMons[battler].maxHP > gBattleMons[battler].hp && (gBattleMons[battler].species == SPECIES_DEWGONG))
                    {
                        BattleScriptPushCursorAndCallback(BattleScript_RainDishActivates);
				...
				case ABILITY_FORECAST:
                    ...
                    if ((gBattleMons[battler].status1 & STATUS1_ANY) && gBattleWeather & B_WEATHER_RAIN && gBattleMons[battler].species == SPECIES_CASTFORM && (Random() % 1) == 0)
                    {
                        if (gBattleMons[battler].status1 & (STATUS1_POISON | STATUS1_TOXIC_POISON))
                            StringCopy(gBattleTextBuff1, gStatusConditionString_PoisonJpn);
                        if (gBattleMons[battler].status1 & STATUS1_SLEEP)
        ...
        case ABILITYEFFECT_ABSORBING: // 3
            if (move)
            {
                ...
                case ABILITY_FORECAST:
					...
					break;
                case ABILITY_DROUGHT:
                    if ((gBattleMons[battler].species == SPECIES_GROUDON && (gBattleWeather & B_WEATHER_SUN_PERMANENT)) && moveType == TYPE_WATER)
                    {
                            if (gProtectStructs[gBattlerAttacker].notFirstStrike)
                                gBattlescriptCurrInstr = BattleScript_BRNPrevention;
                            else
                    ...
                    break;
                case ABILITY_DRIZZLE:
                    if ((gBattleMons[battler].species == SPECIES_KYOGRE  && (gBattleWeather & B_WEATHER_RAIN_PERMANENT)) && moveType == TYPE_FIRE)
                    {
                            if (gProtectStructs[gBattlerAttacker].notFirstStrike)
                                gBattlescriptCurrInstr = BattleScript_BRNPrevention;
                            else
				...
				case ABILITY_RAIN_DISH:
                    if (moveType == TYPE_WATER && gBattleMoves[move].power != 0 && (gBattleMons[battler].species == SPECIES_JYNX || gBattleMons[battler].species == SPECIES_SMOOCHUM))
                    {
                        if (gProtectStructs[gBattlerAttacker].notFirstStrike)
                            gBattlescriptCurrInstr = BattleScript_MoveHPDrain;
                        else
        ...
        case ABILITYEFFECT_IMMUNITY: // 5
            for (battler = 0; battler < gBattlersCount; battler++)
            {
				...
				case ABILITY_SHIELD_DUST:
                    if (gBattleMons[battler].status1 & (STATUS1_BURN | STATUS1_FREEZE | STATUS1_PARALYSIS | STATUS1_POISON | STATUS1_TOXIC_POISON | STATUS1_TOXIC_COUNTER) || gBattleMons[battler].status2 & (STATUS2_INFATUATION | STATUS2_CONFUSION))
                    {
                        StringCopy(gBattleTextBuff1, gStatusConditionString_BurnJpn);
                        effect = 1;
                    }
                ...
                case ABILITY_HYPER_CUTTER:
                    if ((gBattleMons[battler].species == SPECIES_GLIGAR) && gBattleMons[battler].status1 & (STATUS1_POISON | STATUS1_TOXIC_POISON | STATUS1_TOXIC_COUNTER))
                    {
                        StringCopy(gBattleTextBuff1, gStatusConditionString_PoisonJpn);
                        effect = 1;
                    }
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battlerId].hp)
        {
            ...
            case HOLD_EFFECT_HEART_SCALE:
                if ((gBattleMons[battlerId].species == SPECIES_LUVDISC) && gBattleMons[battlerId].hp < gBattleMons[battlerId].maxHP && !moveTurn)
                {
            ...
            case HOLD_EFFECT_TOXIC_ORB:
                if ((gBattleMons[battlerId].species == SPECIES_GLIGAR || gBattleMons[battlerId].ability == ABILITY_EFFECT_SPORE) && gBattleMons[battlerId].hp < gBattleMons[battlerId].maxHP && !moveTurn)
                {
                    gBattleMoveDamage = gBattleMons[battlerId].maxHP / 8;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
        ...
        if (gBattleMoveDamage)
        {
            ...
            case HOLD_EFFECT_FLAME_ORB:
                    ...
                    && gBattleMons[gBattlerAttacker].hp != 0)
                {
                    ...
                    gSpecialStatuses[gBattlerTarget].shellBellDmg = 0;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_FlameOrb;
                    effect++;
                }
                break;
