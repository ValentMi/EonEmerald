...
//static void ShuffleStatArray(u8* statArray); //Three Perfect IVS
...
static const u8 sMonFrontAnimIdsTable[NUM_SPECIES - 1] =
{
    ...
    [SPECIES_DRAGONITE - 1]   = ANIM_GROW_VIBRATE,
...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
	...
	
	{
		...
		if (gMapHeader.mapLayoutId == LAYOUT_METEOR_FALLS_B1F_2R || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_117) //(SPECIES_LUNATONE || SPECIES_SURSKIT || SPECIES_MASQUERAIN || SPECIES_ROSELIA)
		{
			SetBoxMonData(boxMon, MON_DATA_MET_GAME, &gGameVersionSapphire);
		}
		else if ((gMapHeader.regionMapSectionId == MAPSEC_ROUTE_114 && (gMapHeader.weather == WEATHER_SHADE)) || gMapHeader.mapLayoutId == LAYOUT_VICTORY_ROAD_B1F || gMapHeader.mapLayoutId == LAYOUT_MT_PYRE_EXTERIOR)//(SPECIES_ZANGOOSE || SPECIES_MEDITITE || SPECIES_MEDICHAM)
		{
			SetBoxMonData(boxMon, MON_DATA_MET_GAME, &gGameVersionRuby);
		}
		else if (gMapHeader.mapLayoutId == LAYOUT_LOST_CAVE_ENTRANCE || gMapHeader.regionMapSectionId == MAPSEC_POKEMON_MANSION || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_14 || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_8 || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_23 || gMapHeader.regionMapSectionId == MAPSEC_MONEAN_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_LIPTOO_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_WEEPTH_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_DILFORD_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_SCUFIB_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_RIXY_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_VIAPOIS_CHAMBER || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F4 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F3 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F2 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F1 || gMapHeader.mapLayoutId == LAYOUT_KINDLE_ROAD || gMapHeader.mapLayoutId == LAYOUT_TREASURE_BEACH || gMapHeader.mapLayoutId == LAYOUT_CAPE_BRINK || gMapHeader.mapLayoutId == LAYOUT_BOND_BRIDGE || gMapHeader.mapLayoutId == LAYOUT_THREE_ISLE_PORT || gMapHeader.mapLayoutId == LAYOUT_RESORT_GORGEOUS || gMapHeader.mapLayoutId == LAYOUT_WATER_LABYRINTH || gMapHeader.mapLayoutId == LAYOUT_FIVE_ISLE_MEADOW || gMapHeader.mapLayoutId == LAYOUT_RUIN_VALLEY || gMapHeader.mapLayoutId == MAPSEC_SEVAULT_CANYON || gMapHeader.mapLayoutId == LAYOUT_CANYON_ENTRANCE || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_BOTTOM || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_TOP || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_LADDER_ROOM1 || gMapHeader.mapLayoutId == LAYOUT_SHOAL_CAVE_LOW_TIDE_LOWER_ROOM || gMapHeader.regionMapSectionId == MAPSEC_MT_EMBER || gMapHeader.regionMapSectionId == MAPSEC_POWER_PLANT || gMapHeader.regionMapSectionId == MAPSEC_PATTERN_BUSH || gMapHeader.regionMapSectionId == MAPSEC_CERULEAN_CAVE || gMapHeader.regionMapSectionId == MAPSEC_KANTO_SAFARI_ZONE) //(SPECIES_MISDREAVUS || SPECIES_SEEL || SPECIES_DEWGONG || SPECIES_SNEASEL || SPECIES_DRAGONAIR || SPECIES_DRATINI || SPECIES_POLIWAG || SPECIES_SLOWPOKE || SPECIES_CHANSEY || SPECIES_KANGASKHAN || SPECIES_TAUROS || SPECIES_PINSIR || SPECIES_VENONAT || SPECIES_EXEGGCUTE || SPECIES_NIDORINA || SPECIES_NIDORINO || SPECIES_PRIMEAPE || SPECIES_PARASECT || SPECIES_KADABRA || SPECIES_ALAKAZAM || SPECIES_MACHOKE || SPECIES_MACHAMP || SPECIES_ARTICUNO || SPECIES_ZAPDOS || SPECIES_MOLTRES || SPECIES_MEWTWO || SPECIES_RAIKOU || SPECIES_ENTEI || SPECIES_SUICUNE || SPECIES_LUGIA || SPECIES_HO_OH)
		{
			SetBoxMonData(boxMon, MON_DATA_MET_GAME, &gGameVersionFireRed);
		}
		else if (gMapHeader.mapLayoutId == LAYOUT_LOST_CAVE || gMapHeader.regionMapSectionId == MAPSEC_POKEMON_TOWER || gMapHeader.regionMapSectionId == MAPSEC_SEAFOAM_ISLANDS || gMapHeader.mapLayoutId == LAYOUT_WATER_PATH || gMapHeader.regionMapSectionId == MAPSEC_TANOBY_RUINS || gMapHeader.regionMapSectionId == MAPSEC_MEMORIAL_PILLAR) //SPECIES_SCYTHER || SPECIES_DELIBIRD || SPECIES_MURKROW)
		{
			SetBoxMonData(boxMon, MON_DATA_MET_GAME, &gGameVersionLeafGreen);
		}
		else
		{
...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (attackerHoldEffect == HOLD_EFFECT_WISE_GLASSES)
        spAttack = (110 * attack) / 100;
	if (attackerHoldEffect == HOLD_EFFECT_ELECTIRIZER && attacker->species == SPECIES_ELECTABUZZ)
        attack = (150 * attack) / 100;
	if (attackerHoldEffect == HOLD_EFFECT_MAGMARIZER && attacker->species == SPECIES_MAGMAR)
        spAttack = (150 * attack) / 100;
    ...
    if (attackerHoldEffect == HOLD_EFFECT_STICK && attacker->species == SPECIES_FARFETCHD)
        attack *= 2;
    if (attackerHoldEffect == HOLD_EFFECT_LUCKY_PUNCH && attacker->species == SPECIES_LEDIAN)
        attack *= 2;
    if (attackerHoldEffect == HOLD_EFFECT_LIGHT_BALL && (attacker->species == SPECIES_PIKACHU || attacker->species == SPECIES_RAICHU))
        spAttack *= 2;
    ...
    if (attackerHoldEffect == HOLD_EFFECT_THICK_CLUB && (attacker->species == SPECIES_CUBONE || attacker->species == SPECIES_MAROWAK))
        attack *= 2;
    if (defenderHoldEffect == HOLD_EFFECT_PREVENT_EVOLVE && (defender->species == SPECIES_PRIMEAPE || defender->species == SPECIES_MAGNETON || defender->species == SPECIES_LICKITUNG || defender->species == SPECIES_RHYDON || defender->species == SPECIES_TANGELA || defender->species == SPECIES_MR_MIME || defender->species == SPECIES_SCYTHER || defender->species == SPECIES_ELECTABUZZ || defender->species == SPECIES_MAGMAR || defender->species == SPECIES_EEVEE || defender->species == SPECIES_PORYGON || defender->species == SPECIES_PORYGON2 || defender->species == SPECIES_TOGETIC || defender->species == SPECIES_AIPOM || defender->species == SPECIES_YANMA || defender->species == SPECIES_MURKROW || defender->species == SPECIES_MISDREAVUS || defender->species == SPECIES_GIRAFARIG || defender->species == SPECIES_DUNSPARCE || defender->species == SPECIES_GLIGAR || defender->species == SPECIES_SNEASEL || defender->species == SPECIES_URSARING || defender->species == SPECIES_PILOSWINE || defender->species == SPECIES_STANTLER || defender->species == SPECIES_KIRLIA || defender->species == SPECIES_NOSEPASS || defender->species == SPECIES_ROSELIA || defender->species == SPECIES_DUSCLOPS || defender->species == SPECIES_SNORUNT || defender->species == SPECIES_FARFETCHD || defender->species == SPECIES_MEOWTH || defender->species == SPECIES_QWILFISH || defender->species == SPECIES_CORSOLA || defender->species == SPECIES_LINOONE || defender->species == SPECIES_PIKACHU))
        spDefense = (150 * spDefense) / 100;
    if (defenderHoldEffect == HOLD_EFFECT_PREVENT_EVOLVE && (defender->species == SPECIES_PRIMEAPE || defender->species == SPECIES_MAGNETON || defender->species == SPECIES_LICKITUNG || defender->species == SPECIES_RHYDON || defender->species == SPECIES_TANGELA || defender->species == SPECIES_MR_MIME || defender->species == SPECIES_SCYTHER || defender->species == SPECIES_ELECTABUZZ || defender->species == SPECIES_MAGMAR || defender->species == SPECIES_EEVEE || defender->species == SPECIES_PORYGON || defender->species == SPECIES_PORYGON2 || defender->species == SPECIES_TOGETIC || defender->species == SPECIES_AIPOM || defender->species == SPECIES_YANMA || defender->species == SPECIES_MURKROW || defender->species == SPECIES_MISDREAVUS || defender->species == SPECIES_GIRAFARIG || defender->species == SPECIES_DUNSPARCE || defender->species == SPECIES_GLIGAR || defender->species == SPECIES_SNEASEL || defender->species == SPECIES_URSARING || defender->species == SPECIES_PILOSWINE || defender->species == SPECIES_STANTLER || defender->species == SPECIES_KIRLIA || defender->species == SPECIES_NOSEPASS || defender->species == SPECIES_ROSELIA || defender->species == SPECIES_DUSCLOPS || defender->species == SPECIES_SNORUNT || defender->species == SPECIES_FARFETCHD || defender->species == SPECIES_MEOWTH || defender->species == SPECIES_QWILFISH || defender->species == SPECIES_CORSOLA || defender->species == SPECIES_LINOONE || defender->species == SPECIES_PIKACHU))
    ...
    if (defender->ability == ABILITY_SHELL_ARMOR)
        defense = (120 * defense) / 100;
    if (defender->ability == ABILITY_WATER_VEIL)
	...
	if (attacker->ability == ABILITY_PLUS)
		spAttack = (130 * spAttack) / 100;
	if (attacker->ability == ABILITY_GUTS && ((attacker->status1 || attackerHoldEffect == HOLD_EFFECT_FLAME_ORB)))
	{
		// Ensure the effect is applied once, regardless of overlapping conditions
		attack = (150 * attack) / 100;
	}
	if ((MOVE_FACADE) && (attackerHoldEffect == HOLD_EFFECT_FLAME_ORB || attacker->status1))
	{
		// Apply the effect once, regardless of whether one or both conditions are true
		attack = (150 * attack) / 100;
	}
	if (defender->ability == ABILITY_MARVEL_SCALE && ((defender->status1 || defenderHoldEffect == HOLD_EFFECT_FLAME_ORB)))
	{
		// Ensure the effect is applied once, regardless of overlapping conditions
		defense = (150 * defense) / 100;
	}
...
//        gBattleMovePower /= 4;
    ...
    if (type == TYPE_BUG && attacker->ability == ABILITY_SWARM)
        gBattleMovePower = (128 * gBattleMovePower) / 100;
	...
	//Weather
		...
		if (IS_BATTLER_OF_TYPE(gEffectBattler, TYPE_ICE) && gBattleWeather & B_WEATHER_HAIL)
			defense = (150 * defense) / 100;
		if ((attacker->species == SPECIES_CASTFORM) && gBattleWeather & B_WEATHER_HAIL)
			attack = (150 * attack) / 100;
		if ((attacker->species == SPECIES_CASTFORM) && gBattleWeather & B_WEATHER_HAIL)
			spAttack = (150 * spAttack) / 100;
		if ((defender->species == SPECIES_CASTFORM) && gBattleWeather & B_WEATHER_SANDSTORM)
			spDefense = (150 * spDefense) / 100;
		if ((defender->species == SPECIES_CASTFORM) && gBattleWeather & B_WEATHER_SANDSTORM)
			defense = (150 * defense) / 100;
	//Innates
		if ((attacker->species == SPECIES_PERSIAN || attacker->species == SPECIES_MR_MIME || attacker->species == SPECIES_SCYTHER || attacker->species == SPECIES_SCIZOR || attacker->species == SPECIES_SMEARGLE || attacker->species == SPECIES_HITMONTOP || attacker->species == SPECIES_BRELOOM) && gBattleMoves[move].power <= 60)
			gBattleMovePower = (150 * gBattleMovePower) / 100;
		if (attacker->ability == ABILITY_HYPER_CUTTER)
			attack = (110 * attack) / 100;
		if ((MOVE_SNORE || MOVE_UPROAR || MOVE_HYPER_VOICE) && attacker->species == SPECIES_EXPLOUD)
			gBattleMovePower = (150 * gBattleMovePower) / 100;
		if ((MOVE_RAZOR_LEAF) && attacker->species == SPECIES_VENUSAUR)
			gBattleMovePower = (164 * gBattleMovePower) / 100;
		if ((MOVE_KNOCK_OFF) && defenderHoldEffect != HOLD_EFFECT_NONE)
			gBattleMovePower = (150 * gBattleMovePower) / 100;
...
//CONDITIONAL PSS
    ...
    if ((MOVE_TRI_ATTACK) && attacker->species == SPECIES_TOGETIC || attacker->species == SPECIES_CLEFABLE)
		gBattleMoves[move].category == MOVE_CATEGORY_SPECIAL;
    if ((MOVE_UPROAR) && attacker->species == SPECIES_DELCATTY)
		gBattleMoves[move].category == MOVE_CATEGORY_SPECIAL;
	...
	if ((MOVE_SILVER_WIND) && attacker->species == SPECIES_BUTTERFREE || attacker->species == SPECIES_VENOMOTH || attacker->species == SPECIES_BEAUTIFLY|| attacker->species == SPECIES_DUSTOX || attacker->species == SPECIES_MASQUERAIN || attacker->species == SPECIES_YANMA)
		gBattleMoves[move].category == MOVE_CATEGORY_SPECIAL;
	if ((MOVE_DIVE) && attacker->species == SPECIES_POLIWRATH || attacker->species == SPECIES_CLOYSTER || attacker->species == SPECIES_SEAKING || attacker->species == SPECIES_KINGLER || attacker->species == SPECIES_KABUTOPS || attacker->species == SPECIES_GYARADOS || attacker->species == SPECIES_DRAGONITE || attacker->species == SPECIES_FERALIGATR || attacker->species == SPECIES_AZUMARILL || attacker->species == SPECIES_QUAGSIRE || attacker->species == SPECIES_QWILFISH || attacker->species == SPECIES_SWAMPERT || attacker->species == SPECIES_WAILORD || attacker->species == SPECIES_WALREIN || attacker->species == SPECIES_CRAWDAUNT || attacker->species == SPECIES_HUNTAIL || attacker->species == SPECIES_RELICANTH)
		gBattleMoves[move].category == MOVE_CATEGORY_PHYSICAL;
    ...
    if (IS_MOVE_PHYSICAL(gCurrentMove))
    {
        ...
        if ((attacker->status1 & STATUS1_BURN) && (attacker->ability != ABILITY_GUTS && attacker->ability != ABILITY_HYPER_CUTTER) && (!(MOVE_FACADE)))
		{
			damage /= 2;
		}
...
u16 GetBattleBGM(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
		...
		default:
			...
			if (gMapHeader.regionMapSectionId == MAPSEC_EVER_GRANDE_CITY || gMapHeader.regionMapSectionId == MAPSEC_CERULEAN_CAVE || gMapHeader.mapLayoutId == LAYOUT_LOST_CAVE || gMapHeader.regionMapSectionId == MAPSEC_POKEMON_TOWER || gMapHeader.regionMapSectionId == MAPSEC_SEAFOAM_ISLANDS || gMapHeader.mapLayoutId == LAYOUT_WATER_PATH || gMapHeader.regionMapSectionId == MAPSEC_TANOBY_RUINS || gMapHeader.regionMapSectionId == MAPSEC_MEMORIAL_PILLAR || gMapHeader.mapLayoutId == LAYOUT_LOST_CAVE_ENTRANCE || gMapHeader.regionMapSectionId == MAPSEC_POKEMON_MANSION || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_14 || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_8 || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_23 || gMapHeader.regionMapSectionId == MAPSEC_MONEAN_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_LIPTOO_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_WEEPTH_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_DILFORD_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_SCUFIB_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_RIXY_CHAMBER || gMapHeader.regionMapSectionId == MAPSEC_VIAPOIS_CHAMBER || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F4 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F3 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F2 || gMapHeader.mapLayoutId == LAYOUT_ICEFALL_CAVE_F1 || gMapHeader.mapLayoutId == LAYOUT_KINDLE_ROAD || gMapHeader.mapLayoutId == LAYOUT_TREASURE_BEACH || gMapHeader.mapLayoutId == LAYOUT_CAPE_BRINK || gMapHeader.mapLayoutId == LAYOUT_BOND_BRIDGE || gMapHeader.mapLayoutId == LAYOUT_THREE_ISLE_PORT || gMapHeader.mapLayoutId == LAYOUT_RESORT_GORGEOUS || gMapHeader.mapLayoutId == LAYOUT_WATER_LABYRINTH || gMapHeader.mapLayoutId == LAYOUT_FIVE_ISLE_MEADOW || gMapHeader.mapLayoutId == LAYOUT_RUIN_VALLEY || gMapHeader.mapLayoutId == MAPSEC_SEVAULT_CANYON || gMapHeader.mapLayoutId == LAYOUT_CANYON_ENTRANCE || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_BOTTOM || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_TOP || gMapHeader.mapLayoutId == LAYOUT_NAVEL_ROCK_LADDER_ROOM1 || gMapHeader.mapLayoutId == LAYOUT_SHOAL_CAVE_LOW_TIDE_LOWER_ROOM || gMapHeader.regionMapSectionId == MAPSEC_MT_EMBER || gMapHeader.regionMapSectionId == MAPSEC_POWER_PLANT || gMapHeader.regionMapSectionId == MAPSEC_PATTERN_BUSH || gMapHeader.regionMapSectionId == MAPSEC_CERULEAN_CAVE || gMapHeader.regionMapSectionId == MAPSEC_KANTO_SAFARI_ZONE)
				 return MUS_RG_VS_TRAINER;
			   else
				 return MUS_VS_TRAINER;
        }
