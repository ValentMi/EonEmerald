...
void MPlayStart(struct MusicPlayerInfo *mplayInfo, struct SongHeader *songHeader) //DNS
{
    s32 i;
    u8 unk_B;
    ...
    {
			RtcCalcLocalTime();
			if ((FLAG_SYS_SHOAL_TIDE && IsMapTypeOutdoors(GetCurrentMapType()))) //find a way to make it only for outdoor maps
			{
				mplayInfo->ident++;
				mplayInfo->status = 0;
